...
type
  ...
  private
    ...
    function _setfont(fFamily: TPDFFontFamily; fStyle: TPDFFontStyle;
      fSize: double; force_redev: boolean=false): boolean;
  ...
  public
    ...
    procedure SetFont(fFamily: TPDFFontFamily; fStyle: TPDFFontStyle;
      fSize: double = 0.0; fUnderline: boolean = False; force_redev: boolean=false);
    procedure SetFont(fFamily: TPDFFontFamily; fSize: double = 0.0;
      fUnderline: boolean = False ;force_redev: boolean=false);
    procedure SetFontSize(fSize: double; fUnderline: boolean = False);
    procedure SetUnderline(fUnderline: boolean = False);
    procedure TextWithRotation(x:integer; y:integer; txt:string; txt_angle:double;font_angle:double=0.0);
    procedure Text(vX, vY: double; vText: string);
    procedure Textr(vX, vY: double; vText: string);
    procedure Textc(vX, vY: double; vText: string);
    ...
    procedure Header(var Restore_prev:boolean); Virtual;
...
const
  ...
  FREE_JPDF_PASCAL_VERSION = '1.34 Stable';
...
var
  ...
  vcf:             boolean;
  vunderlined:     boolean;
  need_restore_after_header:boolean;
begin
  //Start a new page
  ...
  vtc         := Self.pTextColor;
  vcf         := Self.pColorFlag;
  vunderlined := Self.pUnderlineFlag;
  if (Self.page > 0) then
  begin
    //Page footer
    Self.InFooter := True;
  ...
  //Set font with force_redev
  SetFont(vfamily, vstyle, vsize,vunderlined, true);
  //Set colors
  if (vdc <> '0 G') then
    _out(vdc);
  ...
  Self.pColorFlag := vcf;
  //Page header
  Header(need_restore_after_header);

  //Part only needed, if header ist overridden
  if need_restore_after_header then
  begin
    //Restore line width
    ...
    //Restore font
    SetFont(vfamily, vstyle, vsize,vunderlined);
    //Restore colors
...
procedure TJPFpdf.SetFont(fFamily: TPDFFontFamily; fStyle: TPDFFontStyle;
  fSize: double; fUnderline: boolean ;force_redev:boolean);
begin
  //Select a font; size given in points
  _setfont(fFamily, fStyle, fSize, force_redev);
...
procedure TJPFpdf.SetFont(fFamily: TPDFFontFamily; fSize: double; fUnderline: boolean;
  force_redev:boolean);
begin
  _setfont(fFamily, fsNormal, fSize, force_redev);
  Self.pUnderlineFlag := fUnderline;
end;
...
procedure TJPFpdf.TextWithRotation(x:integer; y:integer; txt:string; txt_angle:double;font_angle:double=0.0);
var
  txt_dx,txt_dy : double;
  font_dx,font_dy : double;
  sss: string;
begin

    font_angle:=font_angle+90+txt_angle;
    txt_angle:=txt_angle  * 3.1415926535898 / 180.0;
    font_angle:=font_angle* 3.1415926535898 / 180.0;

    txt_dx:=round(cos(txt_angle)*10000)/10000.0;
    txt_dy:=round(sin(txt_angle)*10000)/10000.0;
    font_dx:=round(cos(font_angle)*10000)/10000.0;
    font_dy:=round(sin(font_angle)*10000)/10000.0;

    sss:='BT '+FloatToStr(txt_dx)+' '+FloatToStr(txt_dy)+' '+FloatToStr(font_dx)+' '+FloatToStr(font_dy)+' '+FloatToStr(x)+' -'+FloatToStr(y)+' Tm (' +txt+   ') Tj ET ' ;
    if ((Self.pUnderlineFlag) and (txt <> '')) then
      sss += ' ' + _dounderline(x, y, txt);
...
procedure TJPFpdf.Textr(vX, vY: double; vText: string);
begin
  Text(vX-GetStringWidth(vText),vY,vText);
end;

procedure TJPFpdf.Textc(vX, vY: double; vText: string);
begin
  Text(vX-(GetStringWidth(vText)/2.0),vY,vText);
end;
...
function TJPFpdf._setfont(fFamily: TPDFFontFamily; fStyle: TPDFFontStyle;
  fSize: double; force_redev:boolean): boolean;
var
  vfontname: string;
  vn: integer;
begin
  ...
  //Test if font is already selected  , but not if force_redev, it should be set on new pages.
  if ((Self.cFontFamily = fFamily) and (Self.cFontStyle = fStyle) and
    (Self.cFontSizePt = fSize) and not(force_redev)) then
  begin
    Result := True;
    Exit;
  end;
...
procedure TJPFpdf.Header(var Restore_prev:boolean);
begin
